...
export class GitHubApiService {
  ...
  private async getToken(): Promise<string> {
    if (!this.tokenPromise) {
      this.tokenPromise = this.oauthApi.getAccessToken([
        'repo',
        'workflow',
        'repo_deployment',
      ]);
  ...
  private async handleApiCall<T>(
    ...
    try {
      ...
      if (error.status === 403 && error.response?.headers?.['retry-after']) {
        ...
        throw new GitHubApiError(
          ...
          'SECONDARY_RATE_LIMIT_EXCEEDED',
          {
            retryAfter,
            suggestion:
              "This is GitHub's abuse detection. Please wait before retrying.",
          },
        );
      }
      ...
      if (error.status === 403) {
        ...
        throw new GitHubApiError(message, 403, 'INSUFFICIENT_PERMISSIONS', {
          requiredPermissions: ['repo', 'workflow'],
          suggestion:
            'Ensure you have read access to the repository and workflow permissions',
        });
      }
      ...
      if (error.status >= 500) {
        throw new GitHubApiError(
          ...
          {
            suggestion:
              'This is a GitHub server issue. Please try again in a few minutes.',
          },
        );
      }
      ...
      throw new GitHubApiError(
        error.response?.data?.message ||
          error.message ||
          'An unexpected error occurred while calling GitHub API',
        error.status,
        'GITHUB_API_ERROR',
        {
          originalError: error.message,
  ...
  async triggerDeployment(
    ...
    if (!workflowPath) {
      throw new GitHubApiError(
        ...
        {
          suggestion:
            'Configure a workflow path in the environment configuration',
        },
      );
    }
    ...
    return this.handleApiCall(async () => {
      ...
      const runsBefore = await this.listWorkflowRuns(
        owner,
        repo,
        workflow.id,
        1,
      ...
      await this.triggerWorkflow(owner, repo, workflow.id, ref, {
      ...
      try {
        const runsAfter = await this.listWorkflowRuns(
          owner,
          repo,
          workflow.id,
          5,
  ...
  async getDeploymentStatus(
    ...
    return this.handleApiCall(async () => {
      ...
      const deploymentStatuses = await this.listDeploymentStatuses(
        owner,
        repo,
        latestDeployment.id,
        50,
      ...
      const sortedStatuses = deploymentStatuses.sort(
        (a, b) =>
      ...
      if (finalStatus?.target_url || finalStatus?.log_url) {
        const url = finalStatus.target_url || finalStatus.log_url;
        // Try to extract workflow run ID from target/log URL
        const runIdMatch = url?.match(/\/runs\/(\d+)/);
        if (runIdMatch) {
          workflowRunId = parseInt(runIdMatch[1], 10);
          workflowRunUrl = url || undefined;
        }
      }
      ...
      if (
        finalStatus &&
        (finalStatus.state === 'waiting' ||
          finalStatus.state === 'pending' ||
          finalStatus.state === 'queued')
      ) {
        // Try to get detailed approval information from workflow run
        if (workflowRunId) {
          try {
            // Get pending deployments for this run to get approval details
            const octokit = await this.getOctokit();
            ...
            // Find the pending deployment for this environment
            const pendingDeployment = pendingDeployments.data.find(
              (pending: any) => pending.environment.name === environmentName,
            );

            if (pendingDeployment) {
              // Extract reviewer information as strings (matching interface)
              const requiredReviewers = pendingDeployment.reviewers
                .filter((reviewer: any) => reviewer.type === 'User')
                .map((reviewer: any) => reviewer.reviewer.login);

              const requiredTeams = pendingDeployment.reviewers
                .filter((reviewer: any) => reviewer.type === 'Team')
                .map(
                  (reviewer: any) =>
                    reviewer.reviewer.name || reviewer.reviewer.slug,
                );
              ...
              pendingApproval = {
                ...
                requiredReviewers,
                requiredTeams,
                canApprove: pendingDeployment.current_user_can_approve,
                deploymentUrl:
                  workflowRunUrl ||
                  `https://github.com/${owner}/${repo}/deployments/${latestDeployment.id}`,
                timeoutMinutes:
                  pendingDeployment.wait_timer > 0
                    ? pendingDeployment.wait_timer
                    : undefined,
                waitTimerStartedAt: pendingDeployment.wait_timer_started_at
                  ? new Date(pendingDeployment.wait_timer_started_at)
                  : undefined,
              };
            } else {
              // Fallback to basic approval info if we can't find detailed info
              pendingApproval = {
                deploymentId: latestDeployment.id,
                environment: environmentName,
                version: this.parseVersionFromDeployment(latestDeployment),
            ...
            // If we can't get workflow run details, create basic approval info
            pendingApproval = {
              deploymentId: latestDeployment.id,
              environment: environmentName,
              version: this.parseVersionFromDeployment(latestDeployment),
          ...
          // Fallback when we don't have workflow run ID
          pendingApproval = {
            deploymentId: latestDeployment.id,
            environment: environmentName,
            version: this.parseVersionFromDeployment(latestDeployment),
      ...
      const status: DeploymentStatus = {
        ...
        deployedAt:
          deploymentStatus === 'success' && finalStatus
  ...
  async getDeploymentHistory(
    ...
    return this.handleApiCall(async () => {
      ...
      for (const deployment of deployments) {
        ...
        const finalStates = ['success', 'failure', 'error', 'inactive'];
        let finalStatus = sortedStatuses.find(s =>
        ...
        // Check if this deployment is waiting for approval
        if (
          finalStatus &&
          (finalStatus.state === 'waiting' ||
            finalStatus.state === 'pending' ||
          ...
          // Try to get workflow run ID to check for pending approvals
          let workflowRunIdForApproval: number | undefined;
          const url = finalStatus.target_url || finalStatus.log_url;
          if (url) {
            const runIdMatch = url.match(/\/runs\/(\d+)/);
            if (runIdMatch) {
              workflowRunIdForApproval = parseInt(runIdMatch[1], 10);
            }
          }

          if (workflowRunIdForApproval) {
            try {
              const octokit = await this.getOctokit();
              const pendingDeployments =
                await octokit.rest.actions.getPendingDeploymentsForRun({
                  owner,
                  repo,
                  run_id: workflowRunIdForApproval,
                });

              // If there are pending deployments for this environment, it's waiting for approval
              const hasPendingApproval = pendingDeployments.data.some(
                (pending: any) => pending.environment.name === environmentName,
              );
              ...
              // If we can't check for pending approvals, keep the original status
            }
          }
        }
  ...
  async approveDeployment(
    ...
    return this.handleApiCall(async () => {
      ...
      // Get the deployment to find associated workflow run
      const deployment = await this.getDeployment(owner, repo, deploymentId);

      // Get deployment statuses to find workflow run URL
      const statuses = await this.listDeploymentStatuses(
        owner,
        repo,
        deploymentId,
        10,
      ...
      // Find workflow run ID from the latest status target_url or log_url
      let workflowRunId: number | undefined;
      ...
      if (!workflowRunId) {
        throw new Error('Could not find workflow run ID for deployment');
      }
      ...
      // Find the environment IDs to approve
      const environmentIds = pendingDeployments.data
        .filter(pending => pending.environment.name === deployment.environment)
        .map(pending => pending.environment.id)
        .filter((id): id is number => id !== undefined);

      if (environmentIds.length === 0) {
        throw new Error(
          `No pending deployments found for environment: ${deployment.environment}`,
        );
      }
      ...
      await octokit.rest.actions.reviewPendingDeploymentsForRun({
        ...
        state: 'approved',
        comment: comment || 'Deployment approved via Backstage',
      });
    });
  }
  ...
  async rejectDeployment(
    ...
    return this.handleApiCall(async () => {
      ...
      // Find the environment IDs to reject
      const environmentIds = pendingDeployments.data
        .filter(pending => pending.environment.name === deployment.environment)
        .map(pending => pending.environment.id)
        .filter((id): id is number => id !== undefined);
      ...
      await octokit.rest.actions.reviewPendingDeploymentsForRun({
        ...
        environment_ids: environmentIds,
        state: 'rejected',
        comment: comment || 'Deployment rejected via Backstage',
      });
    });
  }
  ...
  async getPendingDeployments(
    ...
    return this.handleApiCall(async () => {
      ...
      for (const deployment of deployments) {
        ...
        if (
          latestStatus &&
          (latestStatus.state === 'pending' || latestStatus.state === 'queued')
        ) {
